...
- 算错的那句删了。`main.cpp:1014-1018` 现在是: 模型在 `g → −g, A → −A, c_s → −c_s` 下逐字不变,
  ...
  > ⚠ **这两行本身是错的, 留着当标本(2026-09-19 第 4 轮复审指出)。**
  > "反向给的是另一个**正数**"只对 `c_s_z > 0` 成立 —— 那正是当时那句错话的**同一个病**,
  > 只是换了个地方重述一遍(叙述里, 而不是输出里)。`c_s_z = −100, cz = 68.7` 时
  > 约定一 = 168.7、约定二 = **−31.3**(负), **正好相反**。
  > 正确的说法是**关系**: 两支差 `2·c_s_z`（约定二 = 约定一 + `2·c_s_z`）,
  > 谁偏大、谁落到零以下, **都随 `c_s_z` 的符号翻**。
  > 本轮修复已按此改正了 4 处**活的**副本(`SessionReport.h` 输出与注释、用例注释两处);
  > 这两行是**历史叙述**, 按复审意见保留原样、只加此标记 —— 免得下一个人当成结论抄走。
