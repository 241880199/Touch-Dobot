...
- ★ **负对照（必做）**：临时把 `Config::MIN_WARN_MS` 从 50 改成 150，新加的边界用例**必须红** —— 证明它真的在判那个常数，而不是又被余量掩盖。做完还原并用文件哈希核对。
...
### 现状（核过源码）
- `run_tests.bat` 里同一块判定**手抄了 19 遍**（`:66` / `:91` / `:111` / … 含 2026-09-23 新接的六个孤儿套件，`run_tests.bat:394-508` 可见是**逐段抄写**、不是循环生成的）。
...
**也不**把 19 处调用点重构成 `call :judge` —— 那会动一个 `cmd` 语义陷阱密集的文件（见 `cmd-batch-semantics` 那十一类实测），本轮不做，风险收益不划算。
...
### 判据
- 整床：`22 of 22 (ran 20 + not-run 2)`、该套件 `74 passed, 0 failed`、**exit 0**。
...
- 测试床从 `19 of 21` 变成 **`20 of 22 (ran 20 + not-run 2)`**，多出 **74 条**从此每次执行。
