...
1. `test_payload_calibration.cpp` 的 `main()` 分两套，用 `#ifdef PC_PARKED_REPLAY_ONLY` 择一：
   - **默认**（无宏）：跑 **76** 条，**不调用**那条红的；
     （**2026-09-24 订正**：这里原写 `74` —— 那是本批**动手前**的用例数；落地实测见下面「判据」那节。）
   - **parked**（有宏）：**只**跑那条红的。
...
- ★ **负对照一（证明接线真的在携带那 76 条）**：临时去掉 `#ifdef`、让默认 exe 又跑那条红的 ⇒ 主套件回到 **`76/1`**（= 76 passed + 那条按设计红着的）、整床 **exit 1**。做完还原。
  （**2026-09-24 订正**：这里原写 `74 条` / `74/1` —— 落地后默认 exe 的用例数是 **76**，带上那条红的就是 `76/1`。）
...
- 测试床从 `24 of 24 (ran 22 + not-run 2)` 变成 **`26 of 26 (ran 24 + not-run 2)`**，多出 **76 条**从此每次执行。
  （**2026-09-24 订正**：这里原写 `74 条` —— 落地实测是 **76**，见 `test_payload_calibration.exe` 末行。）
