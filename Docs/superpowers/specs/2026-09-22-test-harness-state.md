...
⚠★ **但"`test_noise_probe` 现在能在全量跑里跑成了"这句话，本次【没有端到端验证过】。**
2026-09-22 我**没有**跑全量 `run_tests.bat` —— 那一步属于 `2026-09-22-test-harness-builds` 的 Task 2。
上面给出的只是：**已核实的守卫覆盖面（22/22 个 `build_*.bat`）+ 一条推理**（守卫让
`vcvarsall` 至多真跑一次 ⇒ 溢出条件不再出现）。**那不是跑出来的结果。**
要真的证明它，得跑一次全量，并在日志里看到第 7 步确实出现
`--- Building test_noise_probe ---` + `BUILD_EXIT=0` + `=== test_noise_probe.exe ===` + `5 passed, 0 failed`。
