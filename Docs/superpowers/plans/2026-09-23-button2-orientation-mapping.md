...
## Task 3 的**附加要求**（Task 2 评审转交，2026-09-23）

1. ★ **补一条"能区分限幅语义"的用例**（Task 2 评审：现有 ⑧ 只有单轴，在那个输入下
   "逐分量 ±150°"与"旋转角 ≤150°"**等价** ⇒ 本计划头号改动**没有被任何用例区分** ✗）。
   要求：构造一个**多轴**输入，使 `ΔR_rob` 的**旋转角 > 150°** 而**每个逐分量差都 < 150°**，
   断言 `angDeg(Rt, R_ref) == ORIENT_MAX_OFFSET_DEG`；并按既有 NC1–NC4 的规矩**做一次负对照**
   （把限幅临时换成逐分量版本 ⇒ 该用例必须**红**），把负对照结果写进报告。
   ⚠ **具体数值由实现者算**（用 `angDeg` 自证旋转角确实超限）—— 控制方不预先给一个没验算过的数。
2. ⚠ `button2OrientationTarget` 现在**无条件**用兜底 `M`（`touchToRobotMatrix`），
   而 `CoordinateTransform.h:48-49` 明写"`Calibration::enabled` 时调用者要自己分支"。
   本任务要么**在接线处分支**（`Calibration::enabled` 时用 `Calibration::R` 的 3×3，注意那是含平移的
   4×4 语义 ⇒ 只取旋转块），要么**在 `Button2Mapping.h` 里写明"本函数只用兜底 M，标定路径未接"**
   并记账 —— **二选一，写清选哪个与为什么**。
3. Minor 顺手可做（不做也行，做则记进报告）：`Button2Mapping.cpp` 的 NaN 守卫**没有用例**，
   且它对"`refRobotRpy` 本身非有限"的输入会**照样把 NaN 传出去**（注释里"任何非有限的输入"说过头了）
   ⇒ 要么断言式接受（写明），要么给一个确定的兜底，并覆盖三个入参位置。
