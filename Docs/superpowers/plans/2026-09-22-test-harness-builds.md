...
### Task 4（★ 执行中新增，**由 Task 2 的实现者查出**）：把其余七段的"结果判定"也修活

> **★ 这个 Task 的存在说明本计划的一条核心前提是错的。**
>
> Task 2 的 brief 里我写着："删掉第一段 ⇒ 缺陷 A 随之消失（新段落里的 `ERRORLEVEL` 判断是**顶层语句**，
> 没有这个问题）。" **那句话对【外层】判断成立，对【内层】判断是假的。**
> 实现者实测指出：内层的 `if %ERRORLEVEL% EQU 0 (`（判 exe 的运行结果）**嵌套在 `if (...)` 块里**
> ⇒ 与那个被删掉的第一段一样**在解析时冻结** ⇒ **一个返回 7 的 exe 照样打 `[OK]`**
> （他用两个对照脚本证明了，并且能在 cmd 回显里看到 `if 0 EQU 0 (` 这个痕迹）。
> ⇒ **照我给的模板逐字抄，等于把本计划要消灭的那个沉默原样搬了个家** —— 而且会让 Task 2 的 Step 6 负对照
> 以错误的理由失败。
>
...
⇒ **它们的套件即使失败也永远打 `[OK]`**。而本计划的 Goal 是"让 `run_tests.bat` 真的**报告**每一个套件"
⇒ **Goal 未达成**。所以这不是打磨，是没做完。

**Files:** `Touch_Client/tests/run_tests.bat`（只这一个）

> ### ⚠★★ 2026-09-22 复审推翻了我给的第一版修法 —— `if errorlevel 1` 是**错的**
>
> 我原本让内层改成 `if errorlevel 1`。**复审实测证明那个形式会把"崩掉的套件"判成通过**：
> `if errorlevel N` 的真意是 **errorlevel ≥ N**，而崩溃的进程给出的是**负**退出码
> （`0xC0000005` 读作 `-1073741819`）⇒ `-1073741819 >= 1` 为假 ⇒ 走 `else` ⇒ 打 `[OK]`。
> **控制方已独立复现**（写在块里，先 `cmd /c exit -1073741819`）：
> · `if errorlevel 1` ⇒ **`NEWSTYLE=OK`**（假绿）
> · `if !ERRORLEVEL! EQU 0`（开延迟展开）⇒ **`DELAYED=FAIL`**（正确）
> ⇒ **我这个"修法"把一类假绿换成了另一类**：段错误 / 栈溢出 / `raise()` 掉的套件会进
> `Summary: N suite(s) OK`。
>
> ### ⚠★ 我在这里**又说错了一次**，被实现者实测纠正（2026-09-23 复核过）
> 我接着写了一句："**旧的那个冻结形式反倒是数值比较**（`-1073741819 EQU 0` 为假 ⇒ FAIL），
> 所以在'崩溃'这一个情形上我改得比原来的还差。" —— **这句是错的。**
> 冻结的意思是那个 `%ERRORLEVEL%` 在**解析时**就取成了**构建**的退出码（0）
> ⇒ 内层实际执行的是 `if 0 EQU 0` ⇒ **崩溃的套件在旧形式下同样打 `[OK]`**。
> 我拿"数值比较"去论证，却忘了我论证的对象**恰恰是"那个值被冻结了"**这件事本身。
>
> **三种形式的真实行为（实测）：**
> | 形式 | 崩溃（负退出码） |
> |---|---|
> | 旧（冻结，实为 `if 0 EQU 0`） | `OK` —— 假绿 |
> | `if errorlevel 1`（我规定的） | `OK` —— 假绿 |
> | `if !ERRORLEVEL! EQU 0`（现行的） | `FAIL` ✓ |
> ⇒ **回退到旧形式从来不是选项**；现行形式是三者里唯一正确的。

- [ ] **正确修法**：在 `run_tests.bat` 顶部加 `setlocal enabledelayedexpansion`
      （**先 grep 确认整个文件里没有字面 `!`** —— 开了延迟展开之后 `!` 会被当变量引用），
      然后把**每一段内层**（判 exe 结果）改成 **`if !ERRORLEVEL! EQU 0 (`**
      （`[OK]` 分支 + `else` 放 `[FAIL]`）。`!VAR!` 是**运行时**求值 ⇒ 在块里也正确；
      并且它做的是**数值**比较 ⇒ 负的退出码会被判成失败（已复现）。
- [ ] **外层**（判构建结果）的判断**不动** —— 那些确实是顶层语句，是对的（复审逐行确认 12 个都在第 0 列）。
- [ ] **新增一条负对照（崩溃路径）**：把某段的 exe 换成一个**返回负退出码**的替身
      （例如内部 `cmd /c exit -1073741819`，或直接让替身调用 `abort()`/`raise()`）
      ⇒ 必须打 **`[FAIL]`**。**这一条是本修法的判据** —— 没有它，"崩溃算通过"这个缺陷还会回来。
- [ ] **让测试床能对外报失败**（复审 Important 2）：现在脚本**永远返回 0**，
      而且 `Summary` 只是两个计数器 —— 若将来有人重新加回 `if exist` 之类的跳过，
      `N+M` 会静默变成 11 而没有任何东西报警。所以：
      · 末尾加退出码（⚠ **实际采用的形式比这里原来写的更强**：`endlocal & exit /b %HARNESS_RC%`，
        其中 `HARNESS_RC` 在 `FAILED>0` **或**计数不符时都是非零 ⇒ **计数不符也会返回非零**。
        原来写的 `exit /b %FAILED%` 在"计数不符但 FAILED=0"那一格会返回 **0**，正是那个看不见的洞。);
      · 并断言总数：`set /a TOTAL=PASSED+FAILED` 之后 `if %TOTAL% NEQ 12 echo ... MISMATCH ...`。
        ⚠ 那个 `12` 是**硬编码**的 ⇒ 正确新增第 13 段时会**响亮地**报 MISMATCH
        （朝噪音方向错，不会掩盖东西）。加段时记得改它（注释在源码 `:316-318` 说了）。
- [ ] 验收：正常跑 ⇒ 构建步数仍 **12**、12 个 `BUILD_EXIT=0`、各套件结果不变、
      `Summary: 12 suite(s) OK, 0 suite(s) FAILED`。
- [ ] **负对照至少两段** —— 一段在文件**靠前**、一段**靠后**（不能只测已经测过的那一段）：
      把该段的 exe 指向一个必然返回非零的替身 ⇒ 必须打 `[FAIL]` 且 `Summary` 的 FAILED 计数 +1 ⇒ 还原。
- [ ] 每段还原都要**证明干净**：`git status --porcelain` 空 + 与提交态哈希一致。
