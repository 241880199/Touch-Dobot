...
> `Summary: N suite(s) OK`。而**旧的那个冻结形式反倒是数值比较**（`-1073741819 EQU 0` 为假 ⇒ FAIL），
> 也就是说在"崩溃"这一个情形上，我改的比原来的还差。
> （两处文档还把方向说反了：Task 4 的报告说负码"会被判成 [FAIL]"、文件里的规则说新形式"在块里也正确"。
> 都已作废。）
