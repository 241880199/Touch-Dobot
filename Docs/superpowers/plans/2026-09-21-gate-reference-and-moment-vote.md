...
### Task 8: 收尾一致性 —— 把 z-gap 复测结论写回代码注释 + 标定证据文件搬家（**2026-09-21 追加**）

**为什么有这个任务:** Task 6 把 `Fz` 秩 2 那条前提**复测了、按原样不复现**（写进了
`2026-09-19-remaining-workflow.md`），但**代码里的状态标记仍写着"未复测"**
（`ForceCompensation.cpp` 的 `g_guardVote` 那段）。Task 6 的 brief 禁止改 `.cpp`，所以它只能标注、不能改。
**⇒ 一个状态标记与记录不一致的注释，正是本项目账本反复警告的那种"会撒谎的第二来源"。**

**Files:**
- Modify: `Touch_Client/force/ForceCompensation.cpp`（只改那一段文字，不改掩码、不改任何逻辑）
- Move: 标定证据文件（见下）

- [ ] **Step 1:** 把 `g_guardVote` 那段的 `Fz` 前提标记从"未复测"改成与记录一致，
...
| **★ 重采带 `@720` 的夹具** | **本计划的【收口必做项】，不是可选项。** `tests/fixtures/calib_poses_2026-09-19*.txt` 四份**表头都没有 `F720*` 列**（已核实）—— 它们采于 2026-09-19，而记录程序 2026-09-20 才加 `@720`。于是 `test_runtime_consistency_guard_replay` 的"必须拒绝"那半边**问了数据答不了的问题**，被**有意保留为红**（基线 `test_payload_calibration` 69/1、`test_force_compensation` 37/0）。**在拿到带 `@720` 的夹具之前，不许声称闸门在新参考量上被验证过，也不许说"全绿"。** 重采方式与原采法相同：笔尖悬空、只改姿态、**不动负载**。它同时能回答"那条断言该拒绝还是该放行"—— **正确答案很可能是"放行"**（这些采集真值外力 ≈0），但**没有数据就不能断言**。**⚠ 2026-09-21 Task 8 追加（2026-09-21 复审勘误：原写"新夹具读不进来"，方向说反了）**：夹具【读取器】逐行解析用的是 `mgSplitRow(q, c, 25)`，第三参是**上限** —— 解析到 25 列就停、返回 25。所以**31 列的行返回的就是 25**，而守卫判的正是"是不是 18 / 是不是 25"（`t6Load` 是"不是 18 也不是 25 才拒"，`mgLoad` 是"不是 25 才拒"）⇒ **守卫满足、行被收下，最后六列（`@720`）被【静默丢弃】**。于是重采的 31 列夹具**读得进来、套件照样会过**，而**参考量那一路的列从来没有被消费**（用例里参考量那一侧本来就是现算的）⇒ **收口项会看起来做完了、其实什么也没被验证**。**⇒ 重采时必须同时做到两条（这是收口项的真实要求）**：**①** 读取器改成**响亮拒绝它不认识的列数** —— 解析时**把上限抬高、按真实列数判，高于支持上限就拒**；**②** 让用例去消费**真的**参考量那一列，而不是现算。**⚠ 读取器的改动属于【重采那一次】，需要它自己的复审；本计划【不在本任务里改测试代码】。** 这一条此前无人记录。** |
...
| **★ `Fz` 不投票的依据已被推翻，决定悬空** | 掩码让 `Fz` 不投票，其记录依据是"老参考量的 z 响应秩 2"。**Task 6 在 `@720` 上复测：该形状不复现**（第 2、3 轮决定性；第 1 轮 2~3σ、不作数）。**⚠ 机制要说准：z 行的绝对量几乎没变**（`0.0148–0.0201 kg` → `0.0051–0.0164 kg`），**是 x/y 塌了**（`0.20–0.21` → `0.008–0.029 kg`）⇒ "z 不再被结构性压小"说的是**比值**，**不是 z 被修好了**。**⇒ `Fz` 该不该投票，现在【没有有效依据】，而 Task 6 明确拒绝在静态数据上翻掩码**（那条参考量在【接触状态下】未经验证，`@720` 符号约定也未验证）⇒ **该决定与夹具重采绑定**。**代码里的状态标记仍写"未复测"，由 Task 8 写回。** |

> **⚠ 计数口径（后加，2026-09-21 `54475de`）：** 上表"★ 重采带 `@720` 的夹具"那一行引的基线
> `test_payload_calibration` **69/1**、`test_force_compensation` **37/0** 是**本计划写作时**的数。
> 今天：`test_force_compensation` = **39 passed / 0 failed**、`test_payload_calibration`
> = **71 passed / 1 failed**（那 1 条仍是刻意留红）。
> **本计划里各 Step 的 `Expected: …` 行同理** —— 都是**执行当时**的期望值，不是今天的现状
> （Task 7 Step 5 那条"变为 70/0"**当时就没有实现**：那四份夹具答不了问题，红线仍在 ——
> 更正记在 `Docs/superpowers/specs/2026-09-21-gate-reference-prereq.md` §9.4）。
> ⇒ **引用任何计数前先重跑**，别照抄本计划。
