...
**★ 2026-09-23 复审订正（Important，错在【本节文字】不在代码）**：**别再把 A 段的 sd 去跟 0.028 N 比** ✗ ——
那 0.028 N 是 `filtered`（**传感器 N**）口径，而 A 段印的是 `hapticOut`（**Touch N**，≈ 0.0165×gain ≈ **1.98×**）。
⇒ 一个普通的 0.028 N 残差在 A 段会印成 ≈0.055，**凭增益就能"满足"判据** ⇒ 那条确认**一文不值** ✗。

**⇒ 改后的判据（不跨单位比）**：
- **B 段对绝对参照**：`|filtered 残差| >= 0.20 N` 的帧占比 **≈ 15~25%** ⇒ 门机制成立；
  **远低于 15%** ⇒ **门机制否** ⇒ 回去查零偏吸收那条（或另找）。**别按未成立的机制去改死区** ✗。
- **A 段对 B 段（同一窗口）**：`A.sd / B.sd` 应当 **≈ 当时的净比例**（今天 ≈1.98）。
  ⚠ 这个比**不证明也不否证**任何机制，它只回答一个问题：**输出的抖动里有没有超出"残差 × 增益"的东西** ——
  若比值明显偏离净比例 ⇒ 输出里还有别的来源（那才是新线索）。
- ⚠ **行字母不是同一根轴**（复审 Minor 1）：映射是 `hapticOut[0]←Fx, [1]←Fz, [2]←Fy`
  ⇒ **A 段的 `y` 行其实是传感器 Fz、`a.z` 是 Fy**，而 `FORCE_FEEDBACK_Z_SIGN = 0` ⇒ A.y 恒为 0。
  **逐行对齐读 A/B 会把"被关掉的那一轴"与"活的那一轴"配在一起** ✗ ⇒ 读之前先按上面的对应关系换轴。
