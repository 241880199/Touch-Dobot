...
namespace Config {
    ...
    // ★ 2026-09-21 现场试 0: 用户观察"阻力与【速度】成正比 ⇒ 不是惯量(∝加速度)" —— 那指向
    //   物理拖拽 (伺服滞后 ⇒ 工具被拽 ⇒ 传感器上真的有那股力), 而不是惯量项。
    //   ⇒ 置 0 是【诊断】: comp 会退回"纯静态模型的残差" (@1304 − bF − A·g)。
    //     · 仍随速度增大 ⇒ 力【真的在传感器上】⇒ 不是软件 bug (改演示速度 / 机械臂伺服设置)
    //     · 变平 ⇒ 是惯量项在【注入】它 ⇒ 再回去调那一项
    //   ⚠ 试完按结论改回来或改成别的值 —— 这一行现在【不是最终值】。
    const double FORCE_INERTIA_SIGN = 0.0;    // ⚠ 诊断用临时值 (= 不做惯量补偿)
