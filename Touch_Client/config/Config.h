...
namespace Config {
    ...
    //   false = 关【当前】: 边界【不再以任何方式拦】—— 目标原样下发。
    //
    // 【为什么是一个开关, 而不是删掉那几行】这套边界有【两道执法】, 分在两个文件里:
    //   ① 钳位: RelayCore 的 4 个下发点 (目标位置/姿态路径) —— 把目标夹回盒子里;
    //   ② 拒绝: SafetyPredictor 用【同一个函数】当谓词 (clamped 与 target 不同 ⇒ REJECT +
    //      ERR_SAFETY_BOUNDARY)。⚠ 这一道【不是夹, 是整条命令不执行】。
    //   ⚠★ 【实测核过: ② 今天也是死的】它那两个调用点 (:852 evaluate / :1121
    //     evaluatePositionOnly) 拿到的都是 :849【已经夹过】的值 (tcpCheck 来自 servoCmdX/Y/Z,
    //     而那三个就是从 clamped 抄的) ⇒ 谓词恒为"没有越界" ⇒ 它从不拒绝。
    //     ⚠ 我一度断言"只删 ① 会被 ② 拦住"—— 那是【没核调用图就下的结论】, 已在同一处改回。
    //     记在这里是因为它与会话里那条教训同类: 结论要跟着读过的代码走, 不跟着"应该会这样"走。
    //   ⇒ 两道【仍然一起关】, 但理由不是"否则拦得住", 而是【保持两者一致】: 本开关声称的是
    //     "边界不再以任何方式拦", 而 ② 在调用图变化时 (有人传进一个未夹的目标) 就会活过来。
    ...
    //
    // 【怎么恢复】改成 true 即可 —— 两条执法同时回来, 不需要改别的文件。
