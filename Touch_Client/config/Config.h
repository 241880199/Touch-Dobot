...
namespace Config {
    ...
    //   |EMA(compensated − 判据参考量)| > 该通道的容差时拒绝那一路。当前投票掩码: Fx/Fy 投,
    //   Fz 不投, 而 Mx/My/Mz 【此刻同样投票】(各自按 tol_M 判) —— "力矩不再投票"是 Task 4 的
    //   事, 在那之前只写"拒绝 Fx/Fy 两路"是不准确的 (2026-09-21 复审 Minor 2)。
    ...
    //     不是"已验"。本段不动力矩容差的数值 (力矩不投票的改动在 Task 4)。
    //
    ...
    //   【不】改投票掩码: 复审判定"Fz 不投票"这个取舍本身可接受, 错的是没把代价说出来。
