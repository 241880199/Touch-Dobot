...
namespace Config {
    ...
    //
    // 【门限怎么来的】倾角门限处, 输的那个候选留下的力矩误差 ≈ 2·|p_z|·g·sinθ, 其中
    // p_z = m·com_z 是末端负载的一阶矩。本机台实测 m ≈ 0.409 kg / com_z ≈ 67.9 mm
    // (calib/payload_calib.json), 故 2·|p_z|·g ≈ 2 × 0.0278 × 9.81 = 0.5449 N·m。
    // 于是:
    //     sinθ = 0.5 → 分离度 0.272 N·m → 0.2 的差值门限只买到 |b| <= (0.272-0.2)/2 = 0.036
    //     sinθ = 0.7 → 分离度 0.381 N·m → 同一门限买到 |b| <= (0.381-0.2)/2 = 0.090
    // 所以倾角门限一并提到 0.7 —— 它直接决定差值门限还能容忍多大的零偏。
    // 注意: 上式用的 |b| = 0.029 是别处的一次实测记录, 不是本机台现在测的。实机 |b| 到底
    //   多大必须实测 (机械臂静止时 raw 力矩的模长), 量出来之前别把这组门限当成可信。
