...
namespace Config {
    ...
    // ★★★ 惯量项在补偿式里的【符号】—— 2026-09-21 新增。
    //   补偿式是: comp = @1304 − bF − A·g + FORCE_INERTIA_SIGN × (mass × acc)
    //
    // 【为什么需要它 —— 推导】工具质量 m 在传感器下方, 世界系加速度 a:
    //     传感器作用在工具上的力 S:   S + m·g = m·a    ⇒  S = m·a − m·g
    //     传感器量到的是【工具压在它上面的力】= −S = m·g − m·a
    //   而模型 @1304 = bF + A·g (A·g 就是实测出来的重力项) ⇒
    //     @1304 − bF − A·g ≈ −m·a        ← **惯量已经带着一个负号在里面**
    //   ⇒ 要剥出外力, 必须把它【加】回去: comp = @1304 − bF − A·g 【+】m·a
    //   ⇒ 所以本常数应当是 **+1**。
    //
    // 【从前是什么】代码写的是 `− Fi` 且 `Fi = mass × acc` ⇒ 等价于本常数 = −1 ⇒
    //   **惯量不但没被抵掉, 反而被加了两倍** ⇒ 运动时手上出现一股"阻力"(现场 2026-09-21)。
    //   ⚠ 与调零那个 bug 是【同一类】: currentMassKg 的注释自己写着"残余模型时代 mass 是
    //     【可带符号的】残余质量" ⇒ 那时 `− mass×acc` 的符号可以是对的; 而全量模型下
    //     mass = |det A|^(1/3) 【恒正】⇒ 那个符号没地方来了, 公式却没跟着改。
    //   ⚠ 量级校验: 空中跟随实测 |Fi| 均值 0.023 N, 而运动噪声只比静止升 12~33%
    //     (底噪 0.1 N) ⇒ 与"该项被放了两倍"吻合。
    // ⚠ 保留成常数: 若上机发现方向不对, 改这一个数即可 (但物理推导是很硬的, 不只是试)。
    const double FORCE_INERTIA_SIGN = +1.0;
