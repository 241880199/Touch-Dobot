...
namespace Config {
    ...
    // ★★ 2026-09-24 深夜【位置通道的死区 —— 补上姿态通道早就有的那一件】。
    //   【为什么需要】现场症状：**重压时机械臂左右晃动**（操作员没有施加水平力）。
    //   实测（'n' 探针的波形：目标 vs 实际、同帧、123Hz）：**下发的横向目标每帧在跳
    //   0.2~0.55 mm**，比实际摆动还大 ⇒ 臂是【被我们命令着晃】的，不是它自己的伺服。
    //   来源 = 按压时手的微动（越用力越紧、越大）+ 器件噪声，而**位置通道原本 1:1 直连**
    //   （本文件的 DEADZONE/LPF 那一族常数【全是 ORIENT_* 或力通道的】，位置一个都没有）。
    //   【形状：带记忆的死区 = 静摩擦模型】**不能直接砍每帧增量** —— 那会把"慢速有意的移动"
    //   一起砍掉（0.2 mm/帧 × 100 帧 = 20 mm 的真动作）。正确做法是每轴留一个"尚未传递的
    //   余量"：未达门限 ⇒ 目标不动（吸附）；达到 ⇒ 把累积量**整块**传出并清零
    //   ⇒ 平均速度守恒（无系统性滞后），而手抖永远积不起来。
    //   【代价】极小的有意起步会有一点"涩"（0.4 mm 的起步台阶）—— 与姿态通道同一个取舍。
    //   【判据】重压时目标的逐帧步长 sd 应掉到接近静止水平，且小幅写字仍跟手。
    //   ⚠ 现场先试 0.3 / 0.5 / 0.8 三档找折中（改这一行即可，回滚=一行）。
    // ★★ 2026-09-24 深夜【闭环的修法候选：给触觉输出加粘性阻尼】。
    //   【病】现场：重压时机械臂**左右晃动**。查明是一个**自持环路**：
    //     力推 Touch 手柄 ⇒ 手柄动（实测逐帧 0.025~0.41mm）⇒ 位置映射**忠实转发**
    //     （corr(tgt, dev) 0.95~1.00）⇒ 臂跟着动 ⇒ 臂动又产生横向力 ⇒ 回到第一步。
    //   【为什么不是降增益】力反馈倍率有规格 **1~3**（现 1.98），降下去会越出规格 ✗。
    //   【为什么粘性阻尼合规】规格管的是"力 → 输出"的**比例**；粘性项只对**运动**响应、
    //     静止时贡献恒为 0 ⇒ **不改变静态倍率** ✓。这也正是触觉环路的教科书解法
    //     （压振荡靠**加阻尼**，不是靠降增益）。
    //   【为什么低通不行】环路频率实测 **1~5 Hz**，与"有意的写字动作"**同频段** ⇒ 分不开 ✗。
    //   【量级依据】环路里手柄的速度约 **12~50 mm/s**（实测）；要产生与力本身同量级的阻尼
    //     （~0.2 N）⇒ b ≈ 0.005~0.017 N/(mm/s) ⇒ 取 0.01（现场可试 0.005 / 0.02 / 0.03）。
    //   【单位】N · s/mm（= N 每 (mm/s)）。**回滚 = 设 0**（此时阻尼项恒为 0 ✓）。
    //   ⚠ 判据：重压时机械臂不左右晃，且写字手感仍可用（阻尼会让快速拖动略"黏"）。
    const double TOUCH_VISC_DAMPING = 0.0;   // ★ 默认 0 = 关（回滚）
    //   ⚠★ 【2026-09-24 深夜实测：按 0.01 试过 —— **没效果，而且空载运动手感明显变差**】
    //     现场两条反馈合起来给了一个定量结论：阻尼**确实施加上了**（手感变差即证明 ✓）
    //     却**压不住晃动** ⇒ 那个环路的**增益 > 1** ✗。
    //   幅度账：环路横向力 0.15~0.35 N、手柄有效刚度 ~1 N/mm ⇒ 静态位移只该 0.15~0.35mm，
    //     而实测摆到 1.5~3.5mm（臂实际 sd 1.7~3.9mm）⇒ **放大约 10 倍 = 谐振放大** ✓
    //   ⇒ 结论：**通用粘性阻尼是错的工具** —— 它反着【一切】运动（所以空载拖动变沉 ✗），
    //     而需要削的是**环路那一个频率上的增益**，且不能碰"有意的运动"。
    //   ⇒ 默认 0 关闭；代码与用例保留（"为什么单纯加阻尼不行"的记录）。
    const double TOUCH_POS_DEADZONE_MM = 0.0;   // ★ 默认 0 = 关（直通）
    //   ⚠★ 【2026-09-24 深夜实测：先按 0.4 试过，**没解决问题**】现场："晃动没变"。
    //     原因当天就查明了：抖动不是**位置输入**带来的，而是一个**闭环** ——
    //     力输出推 Touch 手柄 ⇒ 位置映射读到手柄的横移 ⇒ 命令臂跟着动 ⇒ 臂动又产生横向力 …
    //     （判据：把力反馈关掉，机械臂就不晃 —— 现场已确认 ✓）
    //   ⇒ 这个死区修的是"位置通道的抖动"，而实际抖动的**幅度来自环路**（整块的位移，
    //     不是逐帧噪声），死区门限远小于它、且环路直接驱动器件 ⇒ 对它无效。
    //   ⇒ 所以【默认关】。机制与单测（`stictionGate`，5 条）保留 —— 它要修的那类问题
    //     （器件位置上的逐帧噪声）将来若真的出现，改这一行即可（0.3/0.5/0.8 现场试）。
    //   ⚠ 0.0 是安全的直通：`stictionGate` 在 dz=0 时每帧都越过门限 ⇒ 原样传出 ✓
    ...
    //   ⚠★ 【"本增益"在运行时是 ForceTuning::gain(), 不是本常数】—— 见下面 2026-09-22 那一条:
    //     本常数只是出厂默认/兜底, 流水线乘的是 ForceTuning 的运行时值 (可取范围也是那一份)。
    //
    ...
    // ★★★ 横向两路的反馈符号 —— **2026-09-23 起是【逐路】两个常数**（不再是一个共用常数）。
    //   ⚠ 本节下面那些"σ = −1 对三行同时成立"的段落是【历史】：09-23 证明那个前提不成立
    //     （一个常数不可能同时对两路 —— 见本文件后面 FORCE_FEEDBACK_TOUCH_X_SIGN 那一大段）。
    //
    ...
    //   ⚠ 【2026-09-23 起已不成立】原写"代码里写死的那组轴对应等于 Mᵀ 再整体取负 ⇒ L = −Mᵀ" ✗ ——
    //     代码现在是 **+Mᵀ**（X:+1 / Z:−1）、Y 单独关掉（见后面逐路常数那段）。
    //
    // 【σ = −1 这个整体符号从哪来 —— 只有一行有实测锚点, 而它把三行一起定了】
    //   ⛔⛔ 【2026-09-23 订正】这个标题本身就是**被推翻的那步推断**："一行有锚点 ⇒ 三行一起定" ✗
    //     两路要的相对符号**相反**（见下 ⛔⛔ 与后面逐路常数那段）⇒ 下面的推导只作**历史**读。
    //   垂直那一行是唯一被实测钉过的: 压笔 ⇒ comp_z = −1.55 N (─ 见 FORCE_FEEDBACK_Z_SIGN
    //   那三行原始数据), 而现场确认过 **`−fz` 的方向是对的** (用户当时只反对"顶得太狠、
    //   落不了笔", 没说方向反) ⇒ 那一行的整体符号是 −1。
    //   ⛔⛔ 【2026-09-23 这句被推翻】原写"一个坐标系之间的关系没有理由逐轴各有一个不同的整体符号
    //     (M 自己的逐轴翻转已经由 M 表达) ⇒ σ = −1 对三行同时成立" ✗ —— **恰恰相反**：
    //     `Mᵀ` 的逐轴翻转**正是**让横向两路的相对符号【必然相反】的原因（X:+1 / Z:−1）。
    //     09-23 的两份现场报告先后从两侧验证了这一点（见后面 TOUCH_X/Z_SIGN 那段）。
    //   ⛔⛔ 【2026-09-23 订正】原写"✅ 独立第二路支持: 用户报告'方向都反了'…两路证据同向" ✗ ——
    //     **那句"都"按本文件自己的解释很可能含【垂直】轴**（见下面 TOUCH_X/Z_SIGN 那段的处置）
    //     ⇒ 它证明不了横向那两路的符号。⇒ 这条"独立支持"**不成立**，被撤回。
    //
    ...
    //   ⛔⛔ 2026-09-23 现场：【上面那条可证伪检验被**判否**了】——
    //     用户实测（原话）："**往右推 touch 会有往右的力**"，即"顺着推"，不是阻力。
    //     链路复算与之一致：按笔右推 ⇒ 笔受接触力**向左**（Fx<0）⇒ `fx × σ = fx × (−1) > 0`
    //     ⇒ Touch 把右手往**右**推 ⇒ 顺手、不抵抗 ✗。
    //     ⇒ 按上面那句自己的规定（"若仍是顺着推 ⇒ σ 推断错了"），**σ 改为 +1**。
    //     ⛔ 这句在**16 行后**就被取代了：09-23 当晚进一步查明两路要**相反**的相对符号，
    //       于是把共用的 σ 拆成逐路两个常数（跨过这里往下看 `FORCE_FEEDBACK_TOUCH_X_SIGN`）。
    //     ★ 这次证否同时**推翻了"一个 σ 三行通用"这个前提**：垂直那行的 −1 是**实测**的
    //       （压笔 comp_z = −1.55 N），横向要有**相反的**整体符号 ⇒ 说明横向的**轴对应**
    //       与垂直那行不是同一种关系（代码在 ForcePipeline 第 4 步早就标着"轴对应尚未被独立验证"）。
    //     ⇒ 【仍未做、且下一步该做的】横向两路的**逐轴实测**（照垂直那次的法子：单轴施加已知外力、
    //       读 comp 的符号与轴），把"力出现在对的轴上"这件事也钉下来 —— 本行只解决了方向。
    //     ⚠⚠ 【必须记下的一件事：两次现场报告互相矛盾】
    //       2026-09-21 的现场报告是「阻力的方向**都**反了」⇒ 当时据此把 σ 定成 −1；
    //       2026-09-23 的现场报告是「往右推 touch 会有往右的力」⇒ 按同一条检验，−1 是错的。
    //       两次都是**现场测量**，不能都成立 ⇒ 中间必有东西变了。最可能的解释：
    //         · 09-21 那句「都」很可能**把那时的垂直轴也算进去了**（垂直当时还开着、后来才关掉），
    //           而横向本身**在 σ 改成 −1 之后从未被单独复验过**；
    //         · 也可能两次描述的是不同的动作（推 vs 压）。
    //       ⇒ 所以本行的依据只有【2026-09-23 这一条具体的、方向明确的报告】，
    //         **不宣称"09-21 那次错了"** —— 两条都留着，等横向逐轴实测来结。
    //     ★ 为什么仍判断「只是符号错、轴没错」：若【轴对应】错，往右推感到的会是**前后方向**的力；
    //       操作员说的是「**往右**的力」⇒ 轴是斜对的，只有方向反了。
    // ⛔⛔ 2026-09-23（用户："先解决阻力方向"）：**上面那种"一个 σ 给横向两路"的写法【在结构上就是错的】**
    //   —— 不是"符号选错"，是"两路需要的符号【相反】"。
    //
    // 【依据：力是向量，器件系的力 = Mᵀ·(基座系力)，而 M 就是平移路径那张表】
    //     基座 +X（右） → 器件 **+X**  （同号）
    //     基座 +Y（前） → 器件 **−Z**  （★ **反号**）
    //     基座 +Z（上） → 器件 +Y      （同号；那一轴今天整轴关着）
    //   （数值已用 M 实算过：Mᵀ·(1,0,0)=(1,0,0) · Mᵀ·(0,1,0)=(0,0,−1) · Mᵀ·(0,0,1)=(0,1,0) ✓）
    //   ⇒ 横向两路的**相对**符号 —— 这是结构，不是选择 —— 必须是 **X: +1 / Z: −1**。
    //
    // 【为什么这解释了那两份互相矛盾的现场报告】各说一路：
    //     σ=−1（原状）：**X 错、Z 对** ⇒ 操作员感到"顺着推"（2026-09-23 那条）✗
    //     σ=+1（当晚翻的）：**X 对、Z 错** ⇒ 修好一路、弄坏另一路 ✗
    //   ⇒ **一个常数不可能同时对** ✓ 这就是"两份报告都成立"的原因。
    //
    // 【拆分后】整体符号（+1）由 2026-09-23 那条现场实测（往右推应是阻力）定；
    //   **相对**符号（X 与 Z 相反）由 Mᵀ 定。两个来源【不同】，别再合成一个数。
    //   ⚠ 垂直那路（`FORCE_FEEDBACK_Z_SIGN`）的旧锚点说它需要 −1，而 Mᵀ 说 +1 ⇒ **两者矛盾**；
    //     它今天关着（0）⇒ 不挡事，但**重开之前必须做逐轴实测**（见下）。
    //   ⚠ 横向的"**力出现在对的轴上**"这件事**至今没有实测过**（只测过"方向反没反"）——
    //     逐轴实测的办法见 `Docs/superpowers/specs/2026-09-22-on-machine-run-sheet.md` 对应那节。
    // ⚠ **待办（复审提的非阻塞项，上机后再做）**：**"derive, don't type"** ——
    //   `ForcePipeline.cpp` 里把**兜底表的符号写成了字面量**，而那张表在 `Calibration::enabled` 为真时
    //   会改走标定出来的 `R` ⇒ 那时**位置走 R、力仍走兜底符号** ✗，而新加的结构用例**读不到 M、抓不到它**
    //   （今天 enabled 恒假 ⇒ 行为一致，所以不阻塞）。更好的写法：`out = σ · (Mᵀ · compensated)`，
    //   而 **`M` 的共享源【已经存在】**：`relay/CoordinateTransform.h` 的 `touchToRobotMatrix()`
    //   （2026-09-23 由 `9caaff7` 抽出，`Button2Mapping.cpp` / `RelayCore.cpp` / `test_frame_layout.cpp`
    //   都已在用它）⇒ 把那 9 个字面量换成它 + Y 那路一个**显式 override**（0 = 关），约两行。
    //   ⚠★ **精确边界（2026-09-23，别读过头）**：**X 的符号有现场锚点**（09-23 那条"往右推往右的力"）；
    //     **Z 的符号是由 `Mᵀ` 推出的、【从未被任何现场报告约束过】的断言** ⇒ 它要上机按执行单那节
    //     （"右推 ⇒ 应在 Touch X 上感到阻力；前推 ⇒ 应在 Touch Z 上"）**第一次检验**。
    //     ⇒ 别把"两路方向都验过了"读成事实：验过的只有 X 那一路。
    // ⛔【已作废 —— 2026-09-24 晚被操作员的手推翻，原文保留作为"为什么不能只信那个探针"的记录】
    //   （原结论：σ_x = −1 · σ_z = +1）它的推理里有一个【没被检验的 frame 假设】：
    //   把"手感 `A.x` 增大"读成"手被推向右"—— 而 `A.x` 是【器件系】的轴，器件 +X 指向哪边
    //   是【约定】、不是测量；操作员的手不需要那个假设（见下面 2026-09-24 那一节）。
    //   //   **σ_x = −1 · σ_z = +1**（= 原写法 `L = −Mᵀ`）。
    //   //   【判它的量法】同一次动作前后各按一次 `'n'`（窗口 = 自上次 'n' 起），**推着的时候按第二次**:
    //   //     基线窗 (sd 只有 0.04~0.08, 干净) vs 右推窗 (机器人真往 +X 走了 169 mm):
    //   //       通道1 差值 **+0.193**, 越死区占比 **1.94% → 46.85%** ← 横向就是它
    //   //       通道2 差值 −0.001 (不动) · 通道3 −1.156 (推时压得更重 = 按压力)
    //   //     而手感 A.x 的变化是 **+0.481 ⇒ 手被推向【右】= 顺着推** ⇒ X 那一路符号反了 ✗
    //   //   ⇒ 今晚前两次改动（共用 +1、再拆 (+1,−1)）**都把它弄坏了** ✗；本次改回 −Mᵀ。
    //   //   ⚠ 与今晚那条口述报告（"往右推往右的力"）**矛盾** —— 但这次是**成对窗口**（基线干净、
    //   //     激励使通道1 的越界占比翻 24 倍），比口述硬；**最终裁判是操作员的手感** ✓
    //   //   ✅ 注意：**两路仍然【相反】** ✓ —— 今晚那条结构性洞察成立，错的只是整体符号。
    const double FORCE_FEEDBACK_TOUCH_X_SIGN = +1.0;   // 器件 X ← +基座 Fx
    const double FORCE_FEEDBACK_TOUCH_Z_SIGN = -1.0;   // 器件 Z ← −基座 Fy   ★ 与上一行【必然相反】
    // ★★★ 2026-09-24 晚【第四次拐弯 —— 这次由操作员的手定案】：**σ_x = +1 · σ_z = −1**
    //   【手的证词两次一致，且两次指向同一件事】
    //     · 2026-09-23（σ_x = −1）：现场报"顺着推" ⇒ 当时记下的结论就是"**X 错、Z 对**"；
    //     · 2026-09-24 晚（σ_x 仍是 −1）：现场又报"**手被推向顺着推的方向，并且伴随抖动**"。
    //     ⇒ 同一符号、同一症状，两次 ⇒ **σ_x = −1 是错的**；而 09-23 那句"Z 对"给了 σ_z = −1。
    //   【与结构性要求一致】两路必须相反（力是向量 ⇒ 器件系力 = Mᵀ·基座系力）⇒ (+1, −1) ✓
    //     这正是 09-23 `b05f3d7` 那一版（"第二拐"）—— **操作员当时认可过的那一版**。
    //   ⚠ 一条**可证伪的预测**（顺带判"抖动是不是它引起的"）：若"顺着推"是【正反馈】，
    //     它天然会振荡 ⇒ 符号改对之后**抖动应当跟着减弱**。若抖动不动，则抖动另有原因
    //     （下一个嫌疑：重力项的姿态滞后，见 RelayCore.cpp 里那段）。
    //   ⚠ 判据（10 秒）：按住按钮1 往**右**慢慢推 ⇒ 手上应感到**往左顶回来**（阻挡）。
    ...
    //       净比例 ∈ [(3.3/200) × GAIN_MIN, (3.3/200) × GAIN_MAX] = 1.65 ~ 4.95。
    //   实测"往下压"那次: comp_z = −1.55 N ⇒ 若这一轴开着, 手上会是 3.07 N (已经到 3.3N 夹子的 93%)。
    //     ⚠ 3.07 N 这个数是【按默认档的净比例算的】, 增益一拖它就变
    //       (范围下端: 1.55 × 1.65 ≈ 2.56 N; 上端早就打顶)。
    //       而"关掉"这个结论【不随增益变】: 全范围里都是远超死区 (FORCE_RESIDUAL_DEADZONE_N)
    //       的显著常力 —— 下端那 2.56 N 也已经是 3.3N 夹子的 78%。
    ...
    const double FORCE_INERTIA_SIGN = +1.0;   // ★ 2026-09-24 深夜【回滚复原值】。下面这段是两次实测的记录：
    //   【判定过程，值得记下来】① 先按 −1 试（依据是"当年 +1 实测阻力变大"）⇒ 现场报
    //     **"空载时速度快了机械臂会有阻滞感"** ⇒ 那一项**在加力、不是在抵力** ⇒ 符号反 ✗
    //     （而且这正好与**推导**一致：`raw` 里含 −m·a ⇒ 要剥离外力必须**加**回去 ⇒ +1 ✓）
    //   ⇒ ② 翻回 **+1**。
    //   ⚠ **当年那次"实测证明它无关紧要 / +1 阻力变大"是【无效的】**：那时估计器的输入是
    //     GetPose 的 10Hz 阶梯位置 ⇒ `acc ≈ 0` ⇒ `Fi ≡ 0` ⇒ 那一项**从头到尾没被施加过**，
    //     所谓"实测"摸到的是别的东西。**现在位置输入改成同帧 @624（估计器活了），符号才第一次真的可判。**
    //   【★★ 结论（2026-09-24 深夜，两次都试过）】**这条路已判死**：
    //     · 符号取 −1 ⇒ 高速空载有阻滞感 ✗（= 在加力）
    //     · 符号取 +1 ⇒ **阻滞感仍在，而且晃动更剧烈** ✗
    //   ⇒ **两个符号都不对** ⇒ 问题不在符号，在**量纲/坐标系**（`mass` 或 acc 的变换）。
    //   ⇒ 操作员判定"这个方向不行"，已回滚（连同 FORCE_POSE_POS_FROM_624），**不再在此项上猜**。
    //     ⚠ 留档价值：它证明了"让估计器活过来"确实改变了行为（阻滞感出现了），只是那一项的
    //       尺度不对 ⇒ 将来若有人要修它，先量 `mass` 与 acc 变换，别再试符号。
    //   【判据（若将来重启此项）】高速空载挥动时不应有阻滞感 ✓
    //   ⚠★ 上面那段历史里有一句"**实测证明它无关紧要**" —— **那个实测在本问题上是无效的**：
    //     它是在**运动估计器死着**的时候做的（估计器的输入是 GetPose 的 10Hz 阶梯位置 ⇒
    //     实测 |acc| 常年 ≈0.05 m/s² ⇒ `Fi = mass·acc ≈ 0` ⇒ 输入恒为 0 时，符号当然无关 ✗）。
    //   【为什么现在要翻】2026-09-24 查明的环路：臂动 ⇒ 传感器读到**臂自身惯性力**(−m·a)
    //     ⇒ 补偿本该用 `+m·a` 抵掉，但估计器死着 ⇒ **一点没抵** ⇒ 环路闭合（力推手柄→手柄动→
    //     臂动→惯性力→又推手柄），现场表现 = **重压时机械臂左右晃动**（量级：摆 2~4mm @1~5Hz
    //     ⇒ a≈0.5 m/s² ⇒ ×0.42kg ≈ 0.2N = 实测横向力的量级 ✓）。
    //   ⇒ 本次同时做两件事（**必须成对**）：① 位置输入改用同帧 @624（让估计器活过来）
    //     ② 本符号取 −1。⚠ **只做①不做②就是把惯性【加倍】** —— 这正是 2026-09-24 早些时候
    //     那次"换 @624 后抖动变大"的原因（当时符号还是 +1）✓ 有据可查。
    ...
    // ★★ 2026-09-24: 重力项 `A·g` 的**姿态**输入用哪一路。
    //   true  = 用【同一帧】的 `ToolVectorActual @624`（123 Hz）—— 桌面查到的 GetPose() 每
    //           100 ms 才刷新一次、运动实测更慢（~200 ms），关节模式手腕 90°/s 时每步陈旧
    //           9~21°，`A·g` 残差可达 1.55 N（离线复算与实测 |F| 逐个对上）。
    //   false = 回到 GetPose 仪表盘那条（= 改动前的行为，**一行回滚**）。
    //   ⚠ 只作用于**姿态**。**位置**（`pose[0..2]`）**刻意不换**：它喂运动估计器 → 惯量项
    //     `Fi = mass·a`，那是**另一件事** —— 换它会把惯量项从"几乎为 0"（阶梯下二阶差分恒 0）
    //     变成一个每帧都在跳的真实量，而本次要修的只是重力项。2026-09-24 现场一次实测：
    //     连位置一起换之后**抖动变大** ⇒ 那条耦合已经拆掉（这条注释就是它留下的账）。
    // ★★ 2026-09-24 深夜【诊断开关】：按钮1 模式下那个【奇异避免的姿态优化器】是否接管下发姿态。
    //   true（默认，= 改动前的行为）= `RelayCore::sendPosition` 的 Mode 1 分支里每帧调
    //     `SingularityAvoidance::optimizeOrientation(clamped, curJoints)`，成功就**改写**下发姿态；
    //     IK 失败则保持"实测姿态"。
    //   false = **跳过整块** ⇒ 下发姿态就是那个"实测姿态"初值（冻结）⇒ 用来做一次 A/B。
    //   【为什么要试它】现场观察：**离开原来的安全边界/工作范围后抖动加剧**，而这条路径
    //     ① 只在"只按按钮1"时跑（与"按钮1 也抖"对上）② 输出来自雅可比 + 安全梯度，
    //     而那两者**在边界/奇异附近是病态的** ⇒ 每帧算出的纠正量会跳。
    //     ⚠ 已知会放大它的既有缺陷：`computeJointPositions` 的 J2 段缺平移 ⇒ `r_elbow ≡ 0`
    ...
    // ★★ 2026-09-24 深夜【位置专用开关，与上面那条姿态的分开】。
    //   true = 运动估计器的【位置】输入取同帧 @624（123Hz）。
    //   【为什么必须动它】估计器的输入原本是 GetPose 的 10Hz 阶梯位置 ⇒ 实测 |acc| 常年
    //     ≈0.05 m/s² ⇒ `Fi = mass·acc ≈ 0` ⇒ **惯量补偿实际上是死的** ⇒ 臂自身惯性力
    //     (−m·a，环路里 ≈0.2N) 一点没被抵掉（见 FORCE_INERTIA_SIGN 那一段的环路说明）。
    //   ⚠ **必须与 FORCE_INERTIA_SIGN = −1 成对使用**：估计器活过来之后符号才有意义；
    //     只动这一条而符号还是 +1 ⇒ 把惯性【加倍】⇒ 抖动更大（2026-09-24 早些时候实测过 ✓）。
    //   false = 回到 10Hz 阶梯（= 改动前，一行回滚）。
    // ★★ 2026-09-24 深夜【环路第一环的抵消 —— 本轮最后一个候选方案】。
    //   【环路】力推 Touch 手柄 ⇒ 手柄动（实测逐帧 0.1~0.4mm、总共 1.5~3.5mm）⇒ 位置映射
    //     **忠实转发**（corr(tgt,dev) 0.945~1.00）⇒ 臂跟着动 ⇒ 臂动又产生横向力 ⇒ 回到第一步。
    //   【本方案】`x_命令 = x_手柄 − F_施加/k`（k = 手柄有效刚度 N/mm）—— 把"因我们的力而
    //     产生的那段手柄位移"从命令里减掉 ⇒ 直接断开第一环 ⇒ 环路增益按 (1 − k真/k估) 缩减
    //     ⇒ **估不准也有效**（估成 2 倍就把环路增益砍一半）。
    //   【为什么它避开了前两次的坑】
    //     · 力为 0 时该项**恒为 0** ⇒ **空载手感丝毫不变** ✓（粘性阻尼对所有运动都反着来 ✗）
    //     · 完全不碰力输出 ⇒ **倍率 1~3 一点不动** ✓
    //     · 只改位置输入 ⇒ 与惯量项（已判死：两个符号都有阻滞感）无关 ✓
    //   【k 的取值】实测环路横向力 0.15~0.35N、手柄位移 1.5~3.5mm ⇒ k 约 0.1~0.2 N/mm ✗ 太小
    //     （那是**环路放大后**的位移）；**静态**下力 0.2N 推 0.15~0.35mm ⇒ k ≈ 0.6~1.3 N/mm
    //     ⇒ 起点取 **1.0**。⚠ 它与人怎么握有关 ⇒ **现场扫**（0.5 / 1 / 2 / 4；0 = 关）。
    //   【判据】重压时机械臂不左右晃 **且空载手感不变**（力为 0 ⇒ 该项为 0 ✓）。
    //     ⚠ 副作用：接触时命令位置会有一个 `F/k` 的**静态偏移**（0.2N ⇒ 0.2mm）—— 是偏置、不是抖动。
    const double TOUCH_HANDLE_STIFFNESS_N_PER_MM = 0.0;   // ★ 默认 0 = 关（回滚）
    //   ⚠★ 【2026-09-24 深夜实测：按 1.0 试过 —— **空载手感有很大变化** ✗】
    //     我设计时假定"**力为 0 时该项为 0**" —— **那是错的**：空载时 `hapticOut` 并不为 0
    //     （有 ~0.3N 的静态残差 **和** 0.05~0.15N 的**波动**）⇒ `−F/k` 于是把**力里的噪声
    //     直接注进了位置命令** ⇒ 手感发涩 ✓ —— 与粘性阻尼**同一类错误**：**用力去改位置** ✗
    //   ⇒ ★★ 由此得到的、比本条更值钱的结论：**力信号真实但嘈杂** ⇒ **任何"用力去改位置/运动"
    //     的方案都会把力的噪声引进运动** ⇒ **这一整类方案都该放弃** ✓（已试两款：粘性阻尼、
    //     −F/k；病根相同。）
    //   ⇒ 代码与用例保留，回滚 = 设 0（= 恒不施加）。
    const bool   FORCE_POSE_POS_FROM_624 = false;   // ★ 2026-09-24 深夜【回滚】：与 FORCE_INERTIA_SIGN 的那对尝试已判死
    ...
    //         本行**之后**（注释 `:774`/`:788`、定义 `:800`；本行之上一条都没有）。按名字找，别按行号。
    ...
    //     ⚠ 【2026-09-22 订正】"无界"这半句【不成立】: m_targetOrient 由
    //     clampOrientToBounds (RelayCore.cpp) 逐帧钳位 ⇒ 它【有界】(钳位自 2026-07-26
    //     e63c0b3 就在, 即累加式那整段时期都受它管; 且它不受 SAFETY_BOUNDARY_CLAMP_ENABLED
    //     管)。累加是真的, "无界"是错的 ⇒ 症状应描述成"积分到 ±180 后被钉住", 不是"越走越大"。
    //     上面的结论 (改用参照式) 【不变】—— 累加式下任何非零抖动都会被积进去, 这条成立。
    //     完整订正见 main.cpp 里"笔杆姿态"那段注释。
    ...
    //     关节空间路径 (`relay/Button2Joint.cpp`) 用它夹 `out[i] − refJoints[i]`, 即
    //     **关节增量**(度), **不是**末端姿态的偏移。名字照旧、语义分岔 ⇒ 详见
    //     `Button2Joint.h` 的「单位变了」那一段与下面 `BTN2_J*_SIGN` 那一段。
    //     (150° 对末端姿态是保守值, 对关节则宽松得多 —— 它没有出处, 与 `SAFE_R*` 同类欠账。)

    // ★★ 2026-09-23 按钮2 的姿态映射改成【真旋转合成】（纯函数 = relay/Button2Mapping.{h,cpp}）。
    //   **回滚 = 把这个开关翻回 false**（一行），旧路径在 RelayCore 的姿态块里【逐字保留】。
    //
    // 【为什么要改】旧算术把"Euler 角之差"当旋转向量用、再把三个角【逐分量加到】参照姿态上 ——
    //   两者都只在小角度下近似成立，而实际摆幅 ±50~80°、且末端姿态常年贴着 rx≈±180。
    //   Task 2 实测（把旧算式逐字抄成函数、与期望矩阵比）：小角度（≤30°）两法完全一致，
    //   而 90° 摆幅、参照取本机真实的 `(-173,-22,-118)` 时**旧法差 149.235°**。
    //   修法：ΔR_dev = R(cur_stylus)·R(ref_stylus)ᵀ（器件系）→ ΔR_rob = M·ΔR_dev·Mᵀ（相似变换，
    //   M = 器件系→基座系那张表，唯一一份定义见 relay/CoordinateTransform.h）→
    //   R_tgt = ΔR_rob·R(ref_robot)（左乘 ⇒ 在【基座系】里倾斜）⇒ 最后才提取一次 RPY。
    //   顺带把偏移限幅从"逐分量 ±150°"改成"**旋转角** ≤150°" —— 夹的是旋转量而不是表示量。
    //   ⚠ M 本身【没动】：`ω_robot = M·ω_dev` 逐行验算成立（Button2Mapping.h 的推导表），
    //     错的不是表、是算术。
    //
    // 【这个开关【不】管什么（那些语义一律保持原样，见 RelayCore 的姿态块）】
    //   · 逐轴死区 `ORIENT_DEADZONE_DEG` 与偏移低通 `ORIENT_STYLUS_LPF_*` —— 仍作用在
    //     【笔杆偏移】上，两条路径共用（它们【不】进纯函数：已经被现场验证过的语义不该搬家，
    //     搬了还会一起被拖出可测范围）；
    //   · 每帧步长 `ORIENT_MAX_STEP_DEG` 那一段、绝对值钳位 `clampOrientToBounds`(±180)、
    //     NaN 守卫 —— 两条路径共用。
    //     ⚠ 2026-09-23 复审 M1 订正措辞：这里的"共用"是"**每帧都会执行**"，**不是**"两条路
    //       都靠它" —— 下面 `BTN2_JOINT_SPACE_ENABLED` 那条关节空间路径下，这段**仍然每帧在跑**
    //       （它在 `RelayCore::sendPosition` 的分叉【之前】），但它的输出只流向
    //       `app.robotTargetPose`（GUI 画的目标位姿），**对真正下发的值没有影响**。
    //       （原文另一处曾写成"新路径**不调** `clampOrientToBounds`" —— 那是**假的**，
    //        它一直在被调用；那一段代码也**一条没删**，旧路径要它、触觉反馈也挂在里面。）
    //   ⇒ 换了算法的**只有**"把笔杆偏移变成机器人目标姿态"这一步。
    //   ⚠ `ORIENT_GAIN` / `ORIENT_FLIP_R*` **只作用于旧路径**（新路径是旋转合成，逐轴标量增益
    //     与逐轴取反在那个式子里没有对应的位置）。三者现在都是 `1.0` / `+1.0`
    //     （见上面 ORIENT_GAIN 与 ORIENT_FLIP_R* 那两段），⇒ 今天两条路径在这些参数上无差别；
    //     **若哪天要调 ORIENT_GAIN，新路径不会跟着变** —— 那是要另设计的事，不是改个常数。
    //
    // ⚠⚠ 【这个开关守不住什么 —— 必须说清】`RelayCore.cpp` **不被任何测试编译** ⇒ 开关切换的
    //   那段代码**没有自动化用例**。能守它的只有三样：① MSBuild 零 error；② 负对照（往被改的
    //   文件里注入一个语法错误，证明这次构建**确实**在编它）；③ 上机（Task 4 的执行单）。
    //   纯函数那一半有单测（tests/test_button2_mapping.cpp，含一条**能区分**"逐分量 vs 旋转角"
    //   的用例），但"接线接对了没有"只能靠上机。
    ...
    //
    // ⚠ 2026-09-23 修复轮（复审 1×Critical + 3×Important + 2×Minor）在**接线层**加了三条，
    //   **它们同样没有自动化用例**（都被 `RelayCore.cpp` 包着）⇒ 只能靠人读 + 上机：
    //     · C1 **模式锁存**：本开关与按钮1 的组合只在 `onButton2Press` 求一次（成员
    //       `m_btn2JointMode`），整段按住不变 —— 否则按住中途换控制律（松开按钮1 ⇒ ServoJ
    //       把臂拽回按下时的位姿；先按2再按1 ⇒ 翻回 ServoP ⇒ 姿态突变）。
    //       ⚠ 这个开关**不再是**每帧判据 ⇒ 想回滚的人改这里、行为在**下一次按下**生效。
    ...
    //   （计划：`Docs/superpowers/plans/2026-09-23-button2-joint-space.md`）
    //
    // 【为什么会有这条路】旧路把结果表达成**末端 RPY**，而本机末端常年贴着 RPY 的接缝与
    //   奇点（实测 `|rx|>170` 占 85.8%）⇒ 才需要 `clampOrientToBounds` / 逐分量限幅 /
    //   跨接缝回避那一整套，而 09-22 那次**关节超速**正是出在那一层。
    //   改走**关节空间**（厂商的 `ServoJ`）之后：**无 IK、无 RPY、无接缝**。
    //
    // 【源侧的对应关系是【实测】的，不是推导】2026-09-23 晚（操作员摆三个姿势、按 SPACE、
    //   机械臂不动、读笔杆欧拉角）：
    //     前后摆 ⇒ 笔杆 **Rx**（主导 +20.3°）⇒ **J4**
    //     左右摆 ⇒ 笔杆 **Rz**（主导 −34.7°）⇒ **J5**
    //     自转   ⇒ 笔杆 **Ry**              ⇒ **J6**
    //
    // 【下面三个正是【唯一没量过】的那一半，所以被提成常数】
    //   上面量到的是"**哪根轴在动**"；"该往哪边转"（正负）与比例**都还没量过** ⇒
    //   默认 +1，由计划 Task 3 上机逐轴实测后钉死（三种结局：+1 / −1 / 该轴压根没接对）。
    ...
    // ★★ 2026-09-24：源轴口径变了（见 `relay/Button2Joint.cpp` 的那一段）——
    //   从"逐欧拉角分量"改成"**器件系旋转向量的分量**"。三个常数的**含义随之变成**：
    //     器件 X 分量（前后摆）⇒ J4 · 器件 Y 分量（左右摆）⇒ J5 · 器件 Z 分量（自转）⇒ J6。
    //   ⚠ 上表那三行"前后摆⇒Rx / 左右摆⇒Rz / 自转⇒Ry"是**旧口径**的实测（主导欧拉角），
    //     它已经被推翻：三个动作的真实旋转轴在器件系里是 X / Y / Z（2026-09-24 用 30 Hz 全量
    //     笔杆数据实测，三根轴两两夹角 74~83°，且只偏名义轴 6~13°）。
    //   ⚠ 但【J4 那一路的数值没变】：`R·Rx(δ)` 只是把最内层那个欧拉角加 δ ⇒
    //     器件 X ≡ ΔRx 是恒等式 ⇒ 09-23 现场判的"J4 反了"（见下面那行 −1）**直接沿用**。
    //     J5/J6 的输入不是同一个量了（从欧拉 Rz/Ry 换成器件 Y/Z）⇒ **这两个符号要重量**。
    // ★★ 2026-09-24 上机实测（换实现后的第一次）：三个符号**全部钉死**。
    //   操作员按 前后摆 → 左右摆 → 自转 各做一次正反摆，报：**J4 方向对、J5 与 J6 都要翻**。
    //   附带一条同样要紧的判据：**每个动作只动一个关节**（否则他看到的是"串着动"，
    //   而不是干净的方向问题）⇒ 新映射（真 ΔR 器件系旋转向量）的**主判据同日通过**。
    const double BTN2_J4_SIGN = -1.0;   // 器件 X（前后摆）⇒ J4 增量。★ 09-24 实测：**方向正确**
                                        //   （09-23 旧口径下判"反了" ⇒ −1；换实现后这一路输入
                                        //    逐位不变（`绕器件 X ≡ ΔRx` 是恒等式）⇒ 沿用成立，
                                        //    当日实测确认）。
    const double BTN2_J5_SIGN = -1.0;   // 器件 Y（左右摆）⇒ J5 增量。★ 09-24 实测：**要翻**
    const double BTN2_J6_SIGN = -1.0;   // 器件 Z（自转）  ⇒ J6 增量。★ 09-24 实测：**要翻**
    //   ⚠ 三个全 −1 是**实测结果**，不是"统一符号更好看"：它们是三根**独立**的器件轴，
    //     每根轴的正方向由器件装夹/右手系决定，彼此没有理由一致 —— 下次若只动其中一路，
    //     改的也只是那一行。
