...
namespace Config {
    ...
    // 的 RPY 所描述的那个之间差这么一个常量。
    // 2026-09-18 实机数据解出 ≈ +79°; 传感器的安装孔按 45° 分度, 所以先取 90°。
    // ⚠ 暂定值: 改完跑一次 'm'→'s', 拟合残差若从 0.73 N 掉到 0.05 N 量级即确认;
    //   若没掉, 就把这个角扫一遍让数据自己找 (它是可解的常量, 不是猜的)。
    // 唯一消费者: TcpCalibration::gravitySensorFrame (力补偿与负载求解【共用】的那一份实现)。
