...
namespace Config {
    ...
    // ===== CZ 符号实测裁决 =====
    // 符号不再靠估计值猜: 同一个静止姿态下依次下发两个候选, 各读一次残余力矩, 谁的小谁对。
    //
    // 【判"两候选之差", 不判"胜者的绝对值"】探针把每个候选约简成一个【模长】:
    //     rProbe[k] = |b + d_k|      b   = 30004 raw[] 里那个【姿态常数的传感器零偏】
    //                                d_k = 候选 k 留下的残余力矩误差
    // 常数 b 只在 (b + d) - b 里抵消, 在 |b + d| 与 |b| 的比较里【不】抵消。所以原来那条
    // "胜者模长 <= 0.05"实际上是在考 |b| 本身, 而不是考候选对不对: |b| 到了 0.05 的量级,
    // 两个候选就都过不了, 求解会一路失败到 CALIB_MAX_CONSECUTIVE_FAILS 锁死
    // (往安全侧失败, 但也等于废掉了)。
    // b 不是假设值 —— 本仓库就是从 raw 里把它测出来的 (ForceCalibration::finalizeBias),
    // 实测记录 bias_torque_nm ≈ [-0.02, 0.02, 0.005], 即 |b| ≈ 0.029 N·m, 和 0.05 同量级,
    // 远大于设计这类判据时假定的 0.003 N·m 噪声本底。
    // 改判两者的【差值】: b 最多把两个模长各挪 |b|, 差值最多被吃掉 2|b|, 与 |b| 的大小脱钩。
    //
    // 【门限怎么来的】倾角门限处, 输的那个候选留下的力矩误差 ≈ 2·|dp|·g·sinθ, 其中
    // dp = p_true − p_cfg 是【负载一阶矩的真值与当前配置值之差】, p = m·com_z。
    // 【注意不是 2·|p_z|·g】那是个错量 (曾经写在注释和报告里): 它只在"当前配置的 p_cfg = 0"
    //   这个特例下才等于 dp。dp 不是机台常数 —— 它随"当前配的负载离真值多远"变, 配置越接近
    //   真值越小; dp → 0 时两个候选重合, 这条判据什么都判不出来 (那是【已收敛】, 不是失败:
    //   main.cpp 里有一条收敛短路专门认它, 不然刚标定完再按 's' 会被报成"实测分不开"并锁死)。
    // 本机台的实际数字 (没有 calib/payload_calib.json, 只有不加载的 .expired, 所以
    // effective() 回退到种子值):
    //     p_cfg  = 0.66 kg × 80.4 mm / 1000        = 0.053064 kg·m   (种子 0.66 / 80.4)
    //     p_true = 0.409137 kg × 67.86 mm / 1000   = 0.027764 kg·m   (上次实机标定)
    //     dp     = 0.027764 − 0.053064             = −0.025300 kg·m
    //     D = 2·|dp|·g = 2 × 0.025300 × 9.81       = 0.4964 N·m
    // 于是:
    //     sinθ = 0.5 → 分离度 0.4964 × 0.5 = 0.2482 N·m → 0.2 的门限只买到 |b| <= (0.2482-0.2)/2 = 0.0241
    //     sinθ = 0.7 → 分离度 0.4964 × 0.7 = 0.3475 N·m → 同一门限买到 |b| <= (0.3475-0.2)/2 = 0.0737
    // 所以倾角门限一并提到 0.7 —— 它直接决定差值门限还能容忍多大的零偏。0.0737 对记下来的
    // |b| ≈ 0.029 N·m 还有 ~2.5 倍余量; sinθ = 0.5 那行只剩 0.0241, 压不住 |b|。
    // (别把 0.0737 圆整成 0.090 —— 0.090 是拿上面那个错量 0.5449 算出来的。)
    // 注意: |b| = 0.029 是别处的一次实测记录, 不是本机台现在测的。实机 |b| 到底多大必须
    //   实测 (机械臂静止时 raw 力矩的模长), 量出来之前别把这组门限当成可信。
    const double SIGN_PROBE_MIN_RATIO     = 3.0;   // 败者的残余还要比胜者大这么多倍
    const double SIGN_PROBE_MIN_MARGIN_NM = 0.2;   // 两候选残余的【差值】下限 (N·m)
    const double SIGN_PROBE_MIN_SIN_THETA = 0.7;   // 姿态倾角门限 (直接决定上面买到多少容差)
    // 600 → 3000: 实机探针量到的两个候选残余几乎相同 (0.2592 / 0.2591), 怀疑 600ms 不够让
    // 新的 EnableRobot 负载进到 30004 力流里 (要么就是使能态下重发 EnableRobot 不重配负载)。
    // 暂定值, 等这次实机实验的 [Probe] 行说话 —— 别当成标定过的常数。
    // ⚠ 不能取大: 探针跑在 GLUT 主线程上, 本函数阻塞多久, 心跳就停多久。
    //    HEARTBEAT_TIMEOUT_MS = 500, 所以整段探针(两次)必须压在 500ms 以内 —— 但
    //    每次探针本身就要 settle + 一个采样窗口, 本来就超了。实测 3000 时直接
    //    触发 ERR_HEARTBEAT_LOST → FATAL → 下使能。600 时侥幸没触发。
    //    [待办] 探针应改成非阻塞(分帧推进)或显式挂起看门狗, 而不是靠"赌它够快"。
    const int    SIGN_PROBE_SETTLE_MS     = 600;
