...
namespace PayloadCalibration {
    ...
    struct RawFit {
        ...
        // ★ 2026-09-19: A 【不再】只由力通道解出来 —— 力方程与力矩方程现在放进【同一个】加权
        //   最小二乘联合估 (未知量 b_F / A / b_M / c_s; 权重 = 实测逐姿态 var/N 对角)。
        //   构造、权重口径为什么是它、收敛证据、以及【已知代价】都写在 .cpp 的 jointEstimateA
        //   那一段上面; 全表与敏感性见 .superpowers/sdd/joint-A-prod-report.md。三条要记住的:
        //     · 没有【可用的逐姿态噪声】时这一步【不做】, A 退回力通道的线性解 (stderr 会说);
        //     · 力矩方程参与了 A 的确定 ⇒ 一份错的力矩模型会把 A 拖歪 (力通道残差随之涨);
        //     · 力通道残差因此可能比从前大: 18:49:55 上 0.650928 → 2.069381 (门限 2.869044,
        //       仍过) —— 那是"用 A 的自洽性换来的", 不是回归。
        double A[9];
        double bF[3];           // 力零偏 (N)
        ...
        // ★ 2026-09-19: 走联合估计那一条路时, [0..11] 的 (J, σ²) 是【那个 18 参数加权系统】
        //   的 (权重 = 实测 var/N, 所以 σ² = χ²_W/(6n−18) 就是它的过散因子, 模型对时 ≈ 1);
        //   [12..17] 仍出自【给定 A 之后】的 6 参数系统。每个块都出自【产出它的那个系统】——
        //   否则 A 已经换成联合解却还报力通道线性解的 CF, 那个数描述的不是 out.A。
