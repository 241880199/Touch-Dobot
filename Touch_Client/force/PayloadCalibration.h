...
namespace PayloadCalibration {
    ...
    enum ModelFormPolicy {
        ...
        // 【2026-09-20 新增】力通道必须验过且通过; 【力矩通道的失拟不拦整体判决】—— 仍然照实报。
        //
        // 为什么需要它: 实机上力矩分支经常拒绝, 而已查明的成因是 **A_F ≠ A_m**(力通道估出的 A
        //   不是力矩通道想要的那个), 【不是"这批数据脏"】—— 尺子与 cond 都干净时它照样拒。
        //   把这个整体判决当作【安装本地补偿】与【生成下发候选】的门, 会把一批参数很好的采集
        //   也挡在外面。实例 (2026-09-20 20:52): cond 17.49 (历次最好)、力通道 2.422 < 2.927、
        //   尺子 0.0251 N —— 只因力矩 35.97 > 10.86 而整体被拒、什么也装不上。
        //
        // ⚠ 【它不是"降低标准"】, 这三点一个字没动:
        //   · 力通道的检验 (统计量、门限、打印) —— 仍然是硬门, 不通过照样 return false
        //   · 力矩通道的失拟统计量、门限、那句拒绝打印 —— 照算照打
        //   · MODEL_FORM_STATUS 那一层 (尺子不齐) —— 照样先判, 不齐仍然不给参数
        //   变的【只是】力矩没过时整体还拒不拒。
        //
        // ⚠ 用了它, momentFormChecked 【保持 false】—— 力矩那一半没通过, 不许读成"验过了"。
        MODEL_FORM_FORCE_ONLY          = 2,
    };
