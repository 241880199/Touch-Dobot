    ...
    if (in.guard != ForceCompensation::GuardState::OK) {
        ...
        switch (in.guard) {
            ...
            case ForceCompensation::GuardState::INCONSISTENT:
                ...
                break;
            default:
                // ⚠ 【不拿"对不上"的话来兜底】(2026-09-21 复审): 这一支从前就长在 INCONSISTENT
                //   那一格上 (default), 于是 GuardState【以后新增的任何一个值】都会静默继承
                //   "对不上"的措辞 —— 而"原因不同, 该说的话就不同"正是这一整段的全部意义
                //   (报错了原因, 操作员会照着一件不相干的事去忙, 那比不报还坏)。枚举加值时
                //   编译器不替我们发现漏配 (见 ForceCompensation.h 那段记账) ⇒ 这里照实说
                //   【不认识】, 不替一个不认识的状态编一个原因, 也不给它编处置。
                causeLine =
                    "[Force]   ⚠ 本次\"没查\"的原因【不明】: 闸门处在一个本检查不认识的状态"
                    "(GuardState 加了新值而这里没配), 所以这里给不出处置指引。\n"
                    "[Force]     ⇒ 以本段上面 \"[Force] !!\" 那一段现报的原因为准 —— 那一段是"
                    "按状态现算的, 不经过本检查。\n";
                break;
        }
        d.text =
