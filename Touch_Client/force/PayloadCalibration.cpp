...
namespace PayloadCalibration {
    ...
    //     与 moment-limit-prod-report.md §6.2 一致。这一栏的 α 兑换率也仍然只描述
    //     modelFormLimit (力分支仍在用)。
    //
    ...
    //     ⚠ 这一条是【离线报告 §1.2】印的数 (Docs/superpowers/evidence/limit-recalibration-report.md)。
    //     本模块自己的配对复算只复现了其中【噪声模型 A】的那一半 —— 12 格的格均值比
    //     excess_bar÷e_bar = 0.9831~1.0199, 落在设计点 0.977~1.023 区间内; 【B 那一半没复现】
    //     (同一个统计量, 格均值比: 0.3558~0.3769)。出处与口径: D:\tmp\limitprod\emeth.txt 的
    //     【阶段 E】24 行表 (列: geom / R / var / e_bar / excess_bar / …), 每行算 excess_bar÷e_bar,
    //     var=0 的 12 格与 var=1 的 12 格各自取最小/最大 ⇒ 上面的两个区间。
    //     ⚠ 那份文件里能看到的 0.241~0.290 是【另一列】—— 逐次抽样的格中位比 med((S−S0)/e)
    //     的 B 半 (该列整 24 格的范围是 0.241~0.822), 不是上面这个格均值比, 别混用。
    //     没有闭环这是口径差异还是配对法理解偏差 —— 与下面假拒率那条里的 B 行指向同一处,
    //     不据此下结论 (交付记录: moment-limit-prod-report.md §3(5) / §8.1)。
    //   · 假拒率 (α=1e-4, 目标 0.01%, 分母 2e5): 离线报告 §4 那条验收带是 0.0040%~0.1602%
    //     (基线 1.19%~22.70%)。⚠ 【它不是"只对噪声模型 A 成立"的数】—— 那条带是【4 份采集 ×
    //     2 套噪声模型 = 8 格】路线 2 假拒率的逐格范围, 带顶 0.1602% 本身就是其中的 15:25/B
    //     那一格, B 的四行 (0.1602 / 0.0115 / 0.0110 / 0.0195 %) 全落在带内 —— 离线研究
    //     【验证过 B】, 且 B 的最坏格就是那条带的顶。出厂口径 (Σ_A 只含逐姿态噪声) 下的实测是:
    //     8 个出厂格【6 格仍在带内】(A 行 0.0040%~0.0335% 逐格复现; B 行的 15:33/B 0.0220% 与
    //     18:49/B 0.0680% 也在带内), 【2 格超出】—— 15:25/B 的 3.5461% 与 15:30/B 的 0.3725%,
    //     分别是带顶 (0.1602%) 的 22.1× 与 2.33×。即: 出厂把 B 的 σ_sys,F² 那一份漏在 Σ_A
    //     外面 ⇒ 门限偏紧 ⇒ 在 B 假设下比【离线验证过的配置】退化 22.1× (最坏格), 而【不是】
    //     "B 本来就没被验证过、超了不花成本"—— 后一种读法把这次的安全相关退化说轻了。
    //     要收这一份得把 σ_sys,F² 显式加进 Σ_A 并重新验证 —— 本轮【没做】, 理由与非选项见下面
    //     "口径上的两个刻意选择"那一段 (它同时是离线报告的头号敏感项)。
    //     (逐格表与"6 进 2 出"的计数: moment-limit-prod-report.md §6.1 的出厂列;
    //      离线带: limit-recalibration-report.md §4 的 route-2 @1e-4 列。)
    ...
    //     方向是反的】: c0 随 R 单调下降, 向下外推给出【更小】的 c0 ⇒ 门限里 c0·LIMIT_prod
    //     那一项【更小 = 更紧】, 而本模块的安全侧恰恰是"更紧" (见 RAW_MODEL_FORM_ALPHA)。
    //     就 c0 这一项而言, 平夹在 R=5 那一档比向下外推【松】; ⚠ 但这条比较【只在 c0 项上成立,
    //     对整把门 (c0·LIMIT_prod + κ·e) 立不住】—— κ 的外推连方向都没有 (见下一句), κ·e
    //     那一项朝哪边走是未知的, 所以"平夹比外推松 / 紧"对本模块的门【无法断言】。
