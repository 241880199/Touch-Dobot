...
static void test_saturation() {
    ...
    //     softDeadzone(500)=500 → ×(3.3/200)=8.25 → 硬夹到 3.3 → ×SIGN(+1, 器件X那一路) → ×gain
...
static void test_coord_transform() {
    ...
    // hapticOut 的映射: **Fx→Touch X（符号 +1）· Fy→Touch Z（符号 −1）** 再乘净比例 (ratio × gain);
    //                     Fz→Touch Y 那一轴【整轴关掉了】(常量 0)。
    // ★★ 2026-09-23: 横向**由【一个共用符号】拆成【逐路两个】** —— 本行随之钉住新结构。
    //
    // 【为什么必须拆】力是向量 ⇒ 器件系力 = Mᵀ·(基座系力)，M 就是平移路径那张表
    //   (relay/CoordinateTransform.h 的兜底: robot=(touch.x, −touch.z, touch.y)) ⇒
    //     基座+X → 器件 **+X**（同号）· 基座+Y → 器件 **−Z**（**反号**）· 基座+Z → 器件 +Y。
    //   ⇒ 两路的**相对**符号【必然相反】⇒ **任何"一个常数给两路"的写法都必错一路** ✗。
    //   （历史：**2026-07-25 的原始设计**就是 `out[0]=fx; out[1]=fz; out[2]=-fy`，且它的用例断言
    //     `hapticOut[2] < 0` ⇒ 本修与那份设计**一致**；**2026-07-26 那次改成了 `out[2]=+fy`**
...
//   ⚠ **出处**：`relay/CoordinateTransform.h` 的兜底分支（`robot=(touch.x, −touch.z, touch.y)`），
//     即 `robot = M·touch`, `M=[[1,0,0],[0,0,-1],[0,1,0]]`。⚠ 边界：该文件在
//     `Calibration::enabled` 为真时改走标定出来的 `R`（今天没有 calibration.json ⇒ 恒假）；
//     而**本用例钉的是两个字面量，它读不到 M** ⇒ 标定一旦启用、位置走 R 而力仍走兜底符号，
//     本用例【抓不到】。⇒ 那条由 Config.h 的 TOUCH_X/Z_SIGN 那段记账（"derive, don't type" 待办）。
//       基座 +X  ->  器件 +X      (同号)
