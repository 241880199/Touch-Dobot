...
static void test_format_basic() {
    ...
    int n = ForceLogger::formatLine(buf, sizeof(buf), 1234, f, p, 1, a, 0, a);
    CHECK(n > 0);
    CHECK(strcmp(buf,
        "1234,1.000,-2.000,3.500,0.100,0.200,0.300,100.000,200.000,300.000,1.000,2.000,3.000,1,"
        "0.2500,-1.5000,3.0000,0,0.250,-1.500,3.000") == 0);
    CHECK(countCommas(buf) == 20);   // 2026-09-24: 末尾追加 raw_x/y/z (3 列)   // 18 列
    PASS();
}
...
static void test_ff_enabled_is_column_14_not_last() {
    ...
    double a[3] = {0,0,0};
    ForceLogger::formatLine(buf, sizeof(buf), 0, f, p, 0, a, 1, a);   // ff=0, still=1
    // 第 14 个字段: 从第 13 个逗号之后开始
    const char* q = buf;
    for (int i = 0; i < 13; i++) { q = strchr(q, ','); CHECK(q != nullptr); q++; }
    CHECK(*q == '0');                 // ff_enabled = 0
    // ★ 2026-09-24: 末尾追加了 raw_x/y/z 三列 ⇒ 【最后一列不再是 is_still】。
    //   本条的意图是"列的位置" ⇒ 改成显式断言末尾三列就是那三个 raw 值（调用时 raw 传的是 a={0,0,0}）。
    const size_t tailLen = 18;   // ",0.000,0.000,0.000"
    CHECK(strlen(buf) > tailLen);
    CHECK(strcmp(buf + strlen(buf) - tailLen, ",0.000,0.000,0.000") == 0);
    PASS();
}
...
static void test_is_still_matches_input() {
    ...
    double p[6] = {0,0,0,0,0,0};
    double a[3] = {0,0,0};
    ForceLogger::formatLine(bufA, sizeof(bufA), 1, f, p, 1, a, 0, a);
    ForceLogger::formatLine(bufB, sizeof(bufB), 1, f, p, 0, a, 1, a);
...
static void test_acc_precision() {
    ...
    ForceLogger::formatLine(buf, sizeof(buf), 7, f, p, 1, a, 0, a);
...
static void test_format_negative() {
    ...
    ForceLogger::formatLine(buf, sizeof(buf), 999, f, p, 1, a, 0, a);
...
static void test_format_truncation_returns_needed() {
    ...
    double p[6] = {0,0,0,0,0,0};
    double a[3] = {0,0,0};
    int needed = ForceLogger::formatLine(small, sizeof(small), 123456789, f, p, 1, a, 0, a);
...
static void test_full_line_fits_default_buffer() {
    ...
    int needed = ForceLogger::formatLine(buf, sizeof(buf), 9999999999UL, f, p, 1, a, 1, a);
    CHECK(needed > 0);
