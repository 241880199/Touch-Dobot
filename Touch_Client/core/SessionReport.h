...
namespace SessionReport {
    ...
    // ⚠ 【已知缺口 (2026-09-20): 下端点是 0, 而物理上它应该是【转接法兰厚度 t】】
    //   上端 31.5 的来历是"测量原点必须落在传感器【体内】" —— 那是从【机器人法兰面】
    //   量起的 31.5 mm。但传感器并不直接贴在机器人法兰面上: 中间有【转接法兰】, 它把整只
    //   传感器往下推了 t。所以物理上成立的区间是 (t, t + 31.5), 不是 (0, 31.5)。
    //   取下端 = 0 ⇒ 【本判据偏宽松】: 它会把 d ∈ (0, t) 这一段【物理上不可能】的值判成
    //   "在范围内"(假通过)。而闸 1 的职责正是【给 cz 定符号】⇒ 假通过的后果是可能定错符号。
    //   ⚠ t 在全仓库【没有任何图纸或实测数据】: 设计 §6b 自己写着"该厚度目前未知",
    //     Hardware/采购单.md 里连相邻的 7mm 凸台都标着"推算值"。
    //     2026-09-20 用户粗量得 **t ≤ 10 mm** —— 只有上界, 没有更细的值。
    //   本配置下这条缺口【不影响结论】: 实测 d = 68.700 − 54.679 = 14.021 mm ⇒
    //     下端余量 d − t ≥ 4.02 mm、上端余量 ≥ 17.5 mm ⇒ 对任何 t ≤ 10 都真的落在体内。
    //     (符号本来也由 cz_robot = 68.700 > 31.5 钉死: 约定二要入界需 t > 91.9 mm。)
    //   ⇒ 【待办】拿到 t 之后把下端换成 t; 在那之前, 这两行就是"判据的下端点架在一个
    //     未知量上"的账 —— 别把 PAYLOAD_D_MIN_MM = 0.0 读成"t 就是 0"。
