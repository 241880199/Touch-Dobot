...
//   ⇒ 调用方可以把返回值与**参照**逐分量相减，**不会**因为跨过 ±180 接缝而变成 ±360°
//   （本机末端常年贴着 rx≈±180 ⇒ 那是常驻工作点）。
//   ⚠★ 2026-09-23 复审订正**保证的【准确边界】**（原来那句"差永远是这一段手动的量"写松了 ✗）：
//     本函数保证的是 **`|out_i − ref_i| ≤ 180`（逐分量）** —— 而**不是**"差等于手动的量"。
//     例：`ref=(180,0,0)` 时，绕基座 Y 手动转 150° ⇒ 规范分支 `rpy=(45,30,180)`，逐分量差
//     `(−135,+30,+180)`（z 那一路到 180 ⇒ **就在线上**）。
//   ⇒ **再接一层下游**：`out_i > 180` 会被 `clampOrientToBounds` **截到 +180** ⇒ 在接缝上，
//     手沿"+ 方向"的动作会被**静默吞掉**，被吞的范围**可达 `ORIENT_MAX_OFFSET_DEG`（150°）**，
//     **不是**零点几度。
//     ⇒ 上机判读因此要写成「**在接缝处预期【完全不动】，而不是缓慢扫过去**」——
//       见执行单 `Docs/superpowers/specs/2026-09-22-on-machine-run-sheet.md` 的接缝那一节。
//     （这是**既有的**接缝限制、旧路径同样如此；本函数只是把它从"整圈自转"化成"目标不动" ✓。）
