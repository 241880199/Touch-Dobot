    ...
    static void solveAndApply() {
       ...
       // ===== 求解基线 (m_cfg / c_cfg): 机械臂【自报】它当前在用的负载 =====
        ...
        for (int i = 0; i < count; i++) {
            ...
            // 不是旋转。2026-09-19 对那批 7 姿态 (冻结夹具
            // tests/fixtures/calib_poses_2026-09-19.txt —— 与 CalibStore 可写目录里那份
            // 活文件的 12:38 那一次逐行相同) 做自由拟合 (F = b + M·g, M 不限形式) 得到的证据:
...
static void runZeroDriftCheck(bool hasStoredZero) {
    ...
    // ★ 2026-09-21 收口 (最终复审): 上面那句"『没查』必须有句话"【也管本检查自己没跑的这条路】。
    //   它从前一个字符都不打就 return, 于是输出上分不出"查了、没发现问题"与"根本没查" ——
    //   与本检查当初为消除的那条静默路径是同一个病。本支【只报一次】(本函数每帧被调)。
    // ⚠ 【--no-robot 那一支已删】(2026-09-21 收口 Fix 5): 它【到不了】—— 本函数全程序只有
    //   一个调用点, 且那一个调用点本身就落在 `if (!g_noRobot)` 里, 而 g_noRobot 是启动时
    //   解析参数定死的 (运行期不再改)。留一段那样的文字等于替一个不可能发生的模式说话,
    //   而它读起来却像一条活路。(别顺手把"没有机械臂"的沉默记到这里: 它不在本检查里。)
    if (!hasStoredZero) {
