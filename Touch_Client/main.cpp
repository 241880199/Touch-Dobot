...
namespace BiasCheck {
    ...
    // 数值唯一来源: PayloadCalibration.cpp 里 RAW_MODEL_FORM_ALPHA 说明中的蒙特卡洛表
    ...
    static const char* const FALSE_REJECT_RATES =
        "模型正确时的假拒率 (α=0.9999, r=3): R=1 约 2.1%、R=2 0.27%、R=3 0.05%";
    ...
    static void solveAndApply() {
        ...
        if (repeatCount > 0) {
            ...
            if (repeatCount < 2)
                std::cout << "[BIAS] 尺子只有 1 对 (自由度 1) —— 尺子自己不够稳: "
                          << FALSE_REJECT_RATES
                          << "。再【换一个姿态】复采一两对 (保持不动按 'r' + SPACE),"
                          << " 判决才收得紧。" << std::endl;
        } else {
            std::cout << "[BIAS] !! 没有重复姿态对 —— 模型形式检验 (fitRaw) 将【无从判定】并"
                      << "拒给参数。摆好一个姿态按 SPACE 采一次, 【保持不动】按 'r' 再按 SPACE"
                      << " 采一次配成一对; 【每一对换一个姿态】, 建议 2~3 对。" << std::endl;
        }
        // 采样中途不允许求解
        if (sampling) {
    ...
    if (key == 'm' || key == 'M') {
        if (!BiasCheck::mode) {
            ...
            std::cout << "\n[BIAS] Mode ON — 笔尖悬空, 只改姿态(位置尽量不变),"
                      ...
                      << "         ★ 每一对都要【换一个姿态】: 挑 2~3 个姿态, 每个姿态采两次配成"
