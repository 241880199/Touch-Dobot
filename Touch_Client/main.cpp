...
namespace BiasCheck {
    ...
    static void logPoseData()
    {
        ...
        // ⇒ 把 @720 落盘, 是为了【核对闸门的参考量该是谁】。在核对之前, 闸门报的那个 |Δc×F|
        //   只能读成"我们 vs 传感器侧那个值", 不能读成"我们 vs 机械臂的负载模型"。
        //
        // ⚠ 它【一直在采】(accumTcp / biasTcp, 每姿态还在控制台打一行 "@720 F=(...)"),
        //   只是从来没写进文件 —— 所以这不是新增采集, 是补一个漏。
        // 精度取 %.6f: @576 那 6 列是 %.3f (它们只用于 0.1 N 量级的三路裁决); 而 @720 若将来
        //   要当闸门的参考量, 它的力矩列要与 tol_M = 0.03 N·m 比 —— %.3f 的台阶(1e-3)是那个
        //   容差的 3%, 按本文件顶上"每列的分辨率不得成为消费它的统计量的瓶颈"那条规矩, 取 %.6f。
